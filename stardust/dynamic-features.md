...
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | events-feed | Event calendar (Vue `v-calendar` month grid + search form + day list on /calendar) fed by GET `/usta/events?dataPagePath=…&source=contentFragments` | A+CR+F | /calendar | data-fed | self | done (da.live sheet /data/events, 353 rows) | off-origin-data → snapshot in code bus (`data/events.json`), `calendar` block rebuilds month grid + search (client-only filter) | **owner:** keep the AEM content-fragment feed reachable cross-origin (CORS) or sync events to a DA sheet; interim = snapshot dated 2026-09-18T17:10:18Z | feed-captures.json (123 KB, 353 events) |
| 2 | event-cards | "Event Calendar at a Glance" 4-card rail (home? no — /events, /calendar, /about, /play) from the same feed | A | 4 pages | data-fed | self | done (authored rows + sheet top-up) | `event-cards` block: document-first (authored 4 rows) + top-up from `data/events.json` by date | same as #1 | dynamicSurface: /usta/events reach 1/50 (rail is server-rendered on 3 pages, hydrated on calendar) |
| 3 | newsfeed | News listing (/news) fed by GET `/usta/nc/newsfeed?newsType=headline&resultsPerPage=8&pageNo=1` (909 items, paginated) | A+L | /news | index-backed | self | done (/news/query-index.json, 897 rows) | `news-listing` block: authored first 8 rows + `/query-index.json` (news sheet) top-up + "load more" pagination; snapshot `data/newsfeed.json` for the 892 archive pages until wave 2 publishes | none (self) | newsfeed.json 335 KB |
...
| 15 | gpt-ads | Google Publisher Tag ad slots (`googletagservices.com/tag/js/gpt.js`, doubleclick, adtrafficquality) rendering the 728×90 "Advertisement" creative | T | 38 pages | embed-passthrough | self | done (ported with the source ids, 2026-09-18) | `ad-slot` block renders the captured creative as an authored image+link (static); GPT loader wired in `scripts/site-config.js` **disabled** | **owner:** run GPT on the new host? ad unit ids (`div-gpt-ad-1789745137559`) | _dynamics.json tags |
| 16 | onetrust-cmp | OneTrust consent (domain script `0559fedb-…`) | T | 50/50 | embed-passthrough | self | done (ported with the source ids, 2026-09-18) | `site-config.js` entry, disabled until the CMP domain is added for the new host | **owner:** add the aem.page/aem.live/prod domain to the OneTrust script | head.html source |
| 17 | adobe-launch | Adobe Launch (`launch-EN1e11bbf…`) → Adobe Analytics, Target (omtrdc), ECID (demdex), Audience Manager iframe | T | 50/50 | embed-passthrough | self | done (ported with the source ids, 2026-09-18) | `site-config.js` entry, disabled; `digitalData`/`adobeDataLayer` object names recorded | **owner:** property id for the new host, consent gating | _dynamics.json settings objects digitalData, dataLayer |
| 18 | google-tags | Google Analytics / Ads / GTM (dataLayer) | T | 50/50 | embed-passthrough | self | done (ported with the source ids, 2026-09-18) | `site-config.js`, disabled | **owner:** measurement ids | _dynamics.json |
...
## Decision batch — resolved 2026-09-18 (owner: Paolo)

1. **Events feed (#1, #2):** migrated to a da.live sheet — `/data/events` (353 rows; edit at https://da.live/#/aemcoder/sdt-ustanationalcampus/data/events); calendar + event rails read `/data/events.json` from the content bus.
2. **Tags (#15–#18):** all ported with the source ids in `scripts/site-config.js` (OneTrust `0559fedb-…`, Adobe Launch `launch-EN1e11bbf…`, GA4 `G-S2332WYSW3` + `G-0MS4BSS9YJ`, Ads `AW-11095761697`, Facebook pixel `3642094592776600`, GPT `/5681/National_Campus` with the 728×90/320×50 size mapping); verified on the published origin (banner, Launch, gtag, fbq, GPT slot; no console errors). OneTrust still needs the new domain added to the domain script's allow-list for the banner to be served on production.
3. **Query index:** `helix-query.yaml` in the repo is honoured by this site (default index 940 rows, `news` index 897 rows at `/news/query-index.json`).
4. **Redirects:** `/redirects.json` sheet (945 rows, source `/en/home/*.html` → new paths) published on preview and live; `/en/home/play.html` → 301 `/play` verified.
5. **Segmentation (#19):** remains decided-out.
6. **usta.com proxy page (#21):** interim thin page + outbound link (unchanged).
7. **Fonts:** Graphik licence confirmation still with USTA.
