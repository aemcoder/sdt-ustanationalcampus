...
- [pending] Hover dropdowns: `.main-nav-sub` sat at `top: calc(100% + 24px)` under a 24px-tall `<li>` — the pointer left the `li` in the gap and `:hover` collapsed the menu before any sub-link was reachable (every dropdown, deployed build). Fix: an invisible `::before` bridge on the sub-list covering the gap (no geometry change, pixel gate untouched), disabled in the mobile query. Probe: `stardust/scripts/nav-hover-path.mjs` walks the pointer link → first sub-link and clicks. Recurring class in stardust-built chrome — proposed for `skills/deploy/SKILL.md` § 6 + `skills/qa/reference/checks.md` (rendered D).
